...
export default function StationDetail({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 md:absolute md:bottom-4 md:left-4 md:right-auto md:w-96 z-40 p-4 md:p-0 transition-transform duration-300 transform translate-y-0">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl p-5 space-y-4 relative overflow-hidden">
        ...
        <div className="flex items-start justify-between gap-4 pt-1">
          <div>
            ...
            <h3 className="text-lg font-black text-slate-900 dark:text-slate-50">{station.sna}</h3>
          </div>

          <button
            onClick={onClose}
            className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors self-start"
            aria-label="關閉"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="grid grid-cols-3 gap-2 pt-2">
          {/* Rent Box */}
          <div className="flex flex-col items-center justify-center p-3 rounded-2xl border border-emerald-100/30 dark:border-emerald-500/10 bg-emerald-50/50 dark:bg-emerald-500/5 text-center">
            <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1 mb-1">
          ...
          {/* Return Box */}
          <div className="flex flex-col items-center justify-center p-3 rounded-2xl border border-blue-100/30 dark:border-blue-500/10 bg-blue-50/50 dark:bg-blue-500/5 text-center">
            <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400 flex items-center gap-1 mb-1">
              <span>🅿️</span> 可還
            </span>
          ...
          {/* Total Box */}
          <div className="flex flex-col items-center justify-center p-3 rounded-2xl border border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 text-center">
            <span className="text-[10px] font-bold text-slate-500 dark:text-slate-400 flex items-center gap-1 mb-1">
              <span>📊</span> 總車位
            </span>
        ...
        <div className="grid grid-cols-2 gap-2 pt-2">
          <button
            ...
            className="flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-200 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 transition-all cursor-pointer"
          >
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="flex items-center justify-center gap-1.5 py-2.5 px-4 rounded-xl text-xs font-bold text-slate-900 bg-[#FFD700] hover:bg-[#ffdf1a] shadow-md shadow-amber-500/10 transition-all text-center"
          >
