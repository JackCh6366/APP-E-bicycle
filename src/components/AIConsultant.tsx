...
const QUICK_QUESTIONS = [
  ...
  { text: '💰 YouBike 2.0 / 2.0E 租借費率怎麼算？', tag: 'rate' },
  { text: '🌧️ 下雨天騎車有哪些注意事項？', tag: 'safety' },
  { text: '🔑 電子票證（悠遊卡/一卡通）如何註冊？', tag: 'easycard' },
  { text: '🛠️ 借到故障車該如何處理？', tag: 'broken' },
  { text: '🚴 跨縣市騎乘還車有調度費嗎？', tag: 'dispatch' },
];
...
export default function AIConsultant({ currentCityName = '台北市', currentDistrict, selectedStation, stationsInDistrict }: AIConsultantProps) {
  ...
  const handleSend = async (textToSend: string) => {
    ...
    try {
      ...
      // 只有在查詢站點/找車時才注入 Top 5 站點，避免在問費率或操作時被站點清單干擾導致答非所問
...
1. 費率說明：
   ...
   - YouBike 2.0E 電輔車（電力輔助自行車）：前 2 小時每 30 分鐘 20 元；超過 2 小時每 30 分鐘 40 元（無前 30 分鐘免費優惠）。
2. 跨區調度費規則：
   - 台北市 ↔ 新北市：互還免收跨區調度費。
...
3. 租借與註冊方式：
   ...
   - 掃碼租借：下載「YouBike 2.0 官方 APP」，綁定信用卡或行動支付（如 LINE Pay、街口等）後掃描車機螢幕上的 QR Code 借車。
4. 車輛故障與維修：
   - 借車後 5 分鐘內同站還車不計費。
...
3. 服務邊界：
   - 若問題與 YouBike、公共自行車、交通通勤無關，請幽默且有禮地委婉回應，並熱情引導回 YouBike 相關服務。`;
      ...
      >
        ...
        <div className="bg-slate-900 dark:bg-slate-950 p-4 text-white relative">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-medium">即時車況、租借費率與騎乘指南</p>
              </div>
            </div>
          ...
          <div className="mt-2.5 bg-white/10 dark:bg-white/10 rounded-lg p-2 flex items-center justify-between gap-2 text-xs">
            <span className="text-[11px] font-bold text-[#FFD700] shrink-0 flex items-center gap-1">
              <Sparkles className="w-3.5 h-3.5 text-amber-400" /> AI 模型：
            </span>
            <select
          ...
          <div className="mt-2 bg-white/5 dark:bg-white/5 rounded-lg px-2.5 py-1.5 flex items-center justify-between gap-2 text-[10px] text-slate-300">
            <span className="truncate">
              📍 縣市區：<strong className="text-[#FFD700]">{currentCityName} {currentDistrict || '全區'}</strong>
              {selectedStation && (
                <>
                  {' / '} 選取：<strong className="text-white">{selectedStation.sna}</strong>
                </>
              )}
            </span>
            <span className="shrink-0 text-[9px] bg-slate-800 text-slate-400 px-1 py-0.5 rounded">
              {stationsInDistrict.length} 站即時資料
            </span>
          </div>
        </div>
          ...
          {isLoading && (
            <div className="flex gap-2.5 justify-start">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl rounded-tl-none px-3.5 py-3 shadow-xs border border-slate-200/40 dark:border-slate-800/60 flex items-center gap-2">
                ...
                <span className="text-[11px] text-slate-700 dark:text-slate-200 font-semibold animate-pulse">AI 客服正在為您整理精確回覆...</span>
              </div>
            </div>
          )}
        ...
        {messages.length < 5 && (
          <div className="px-4 py-2 bg-slate-50/50 dark:bg-slate-950/40 border-t border-slate-100 dark:border-slate-800/40">
            <span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider block mb-1.5">常見快捷諮詢</span>
